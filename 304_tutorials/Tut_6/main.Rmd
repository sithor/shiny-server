...
`performance` package. The argument `x` is the Poisson regression model object which will be estimated later.
...
##### **`r Qnum()`**
We saw before that, for the outcome, there's some evidence of over-dispersion. A formal check for this was **negative**, so here a Poisson model is **sufficient**. If the `performance::check_overdispersion()` function returned a significant $P$-value, we would have to think about relaxing the strict assumptions of the Poisson model relating to the mean and variance of the outcome being equal. 

In such a situation a *quasiPoisson* model (please use `fit_qp` as the new model object. Do this by changing `poisson` to `quasipoisson` in the code below and running it.
