...
Look at your first histogram above – for `arf_count`. In the previous section, you estimated the mean of  `arf_count` displayed from using the `skim` function (`0.55`) and compare it with the variance you calculated (`0.0033`).\n\n
Which of the following, if any, is consistent with underdispersion?",
...
question("Your results should point to evidence of underdispersion. Given this observation, what type of regression would improve model fit?",
...
relationships especially between key exposure and outcome variables.
...
Here, we select five variables from `df` and put them into a `data.frame` called `my_data`. The we apply our `PerformanceAnalytics::chart.Correlation()` function to `my_data` to get histograms, scatter plots and Pearson correlation coefficients.
...
question("For `Permethrin_rate`, you should get an `Estimate` value of 3.527e-04. What does this tell us?",
  answer("There is, on average, 3.527e-04 = 0.0003527 more acute rheumatic fever for every one unit increase in permethrin rate", message = "No. 3.527e-04 is measured on a natural-log scale. So you need to exponentiate it to give a meaningful interpretation"),
...
While the exponentiated coefficient don't seem large (1.00035 is a 0.035% change, with a one unit increase of permethrin), the nature of the association may only be understood by also knowing to what extent permethrin dispensing varies in the population. 
...
##### **`r Qnum()`**
We saw before that, for the outcome, there's evidence of under-dispersion. To account for this, we will specify a *quasiPoisson* model (please use `fit_qp` as the new model object. Do this by changing `poisson` to `quasipoisson` in the code below and running it.
...
grade_this_code("Great! Above, we created a model named `fit_qp`. As we want to exponentiate, the logical value for the `exp` argument will be `TRUE`")
```
...
## **Model fit**
How well a model fits the data can be assessed with a **'marginal model plot'**, which shows the relationship between model predicted (model) and observed data (data). In a good fitting model, we expect to see the two lines closely superimposed on each other. Poor fit is identified by seeing separation between the two lines.
