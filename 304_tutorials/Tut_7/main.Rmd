...
Logistic models rely on a mathematical trick to transform the **left-hand**
...
```{r var_str, echo = FALSE}
question("What type of variable is this?",
answer("character", message = "No. try again."),
answer("numeric", message = "No. try again."),
answer("factor", message = "Well done!",  correct = TRUE),
answer("logical (`TRUE`, `FALSE`)", message = "No. try again."),
answer("integer", message = "No. try again."),
  allow_retry = TRUE
)
```
...
answer("P = 0", message = "No. A P-value is *never exactly 0.* There is always a chance (no matter how small), that our observed results (or one more extreme) could have occurred under the null hypothesis"),
  answer("P = 0.0001", message = "No, the P-value is lower than this"),
  answer("P < 0.0001", message = "Yes, this is consistent with the very high Chi-squared statistic (of ~33). It is a convention to report very low *P*-values this way",  correct = TRUE),
  allow_retry = TRUE
)
```
...
Unfortunately many of the epidemiological tools to analyse $n$ x 2 tables fall over
for `SmokeCurrent` and `Marijuana`. This is because they rely on Fisher tests which 
are relatively computationally intensive. We will just use a simple table and chi-square instead.
...
You can use the table to quick mental calculation of risk ratios. For example, you can see that 81%
of current smokers (`SmokeCurrent = "Yes"`) are `Marijuana` users, compared to 42.7% of never smokers 
(`SmokeCurrent = Never`). You can quickly see that the risk ratio is about 81%/42.7% $\approx 2$. The $P$-value for the association is low (< 0.001), so the association between the two behaviours are statistically significant. 

##### **`r Qnum()`**

```{r strength_cigs, echo = FALSE}
question("Is the strength of the association between `Marijuana` and
`SmokeCurrent` overall: weak, medium or strong?",
answer("strong", message = "Correct! risk ratios of 2 for marijuana use among current smokers, compared to never-smokers, is a strong associations, but we will find a stronger one still!", correct = TRUE),
  answer("Weak", message = "No, please try again."),
  answer("moderate", message = "No, please try again."),
  allow_retry = TRUE
)
```
...
```{r hard_drugs_2, echo = FALSE}
question("How would you describe the relationship between hard drugs and marijuana use from the output above?",
  answer("Weak", message = "No, please try again."),
  answer("Moderate", message = "Getting warmer"),
  answer("Strong", message = "Correct! An odds ratio of 27 is super-duper strong!", correct = TRUE),
  allow_retry = TRUE
)
```
...
answer("`HardDrugs_bin`", message = "Yes, the correlation coefficient is greatest!", correct = TRUE),
...
`Marijuana`?",
  ...
  answer("`HardDrugs`, `Race1`, `Gender`, `Depressed`, `SmokeNow` and `Alcohol12PlusYr`", message = "Correct! The P-values for these comparisons are less than 0.05", correct = TRUE),
...
answer("`HardDrugs` contributes more to the outcome (marijuana use) than `SmokeCurrent`", message = "Statement is true as `HardDrugs` has a much longer scale",  correct = TRUE),
...
Here, the $P$-value is 0.91, which is not significant. Therefore, the
