...
Look at your first histogram above – for `arf_count`. In the previous section, you estimated the mean of  `arf_count` displayed from using the `skim` function (`0.55`) and compared it with the variance you calculated (`1.36`).\n\n
Which of the following, if any, is consistent with over-dispersion?",
  answer("Variance > mean", message = "Yes. The variance (1.36) is substantially greater than the mean (0.55).", correct = TRUE),
  answer("Variance < mean", message = "No. This would be evidence of under-dispersion. Please try again"),
  allow_retry = TRUE
)
```
...
question("How would you interpret the scatter plot between the potential confounders and `arf_rate`?",
  ...
  answer("Something else", message = "No, please try again!"),
  allow_retry = TRUE
)
```
...
Was is the **incidence rate ratio** for `I(Permethrin_rate/1000)`? We need to exponentiate the beta-coefficient.
