...
question("**Q1**. Which of these statements about the solid blue line is **true**?",
...
question("**Q2.** Roughtly, what is the average birthweight at 35 weeks gestation?",
...
**Q3.** Run code to produce a scatter plot of the association between gestation period (`Gestation`; independent variable) and birth weight (`Birth_wt`; dependent variable), stratified by ethnic group (`Ethnic`). Use the code format given.
...
question("**Q4.** Which ethnic group has the steepest slope?",
...
question("**Q5.** You should get an `Estimate` value of 192 for Gestation (rounded to the nearest whole number). Interpret the meaning of this figure.",
...
question("**Q6.** What is the average weight of a baby at 40 weeks gestation, predicted by the linear model?",
...
question("**Q7.** You should get 0.4046 for `Multiple R-squared`. What does this tell us?",
  ...
  answer("The model explains 40.46% of the variation in 0.4046", correct = TRUE, message = "Yes, 0.4046 is a proportion – so it’s 40.46%"),
  allow_retry = TRUE
)
```

```{r crude_lr_4, echo=FALSE}
question("**Q8.** Which of the following gives the lower 95% confidence interval for the slop of `Gestation`?",
...
question("**Q9.** Maternal cigarette smoking (vs. non-smoking) is associated with how much mean difference in birth weight, on average, after adjusting for gestational age?",
...
question("**Q10**. Is the `Mother_smoke` variable significantly associated with `Birth_wt`?",
...
**Q11** How much has the $\beta$ coefficient for `Gestation` changed with the addition of `Mother_smoke`?
...
question("**Q12** How much change (minimum) in the $\beta$-coefficient (%) is the minimum likely to represent substantial confounding of the relationship between gestation and birth weight?",
...
**Q13** How would you adjust the code below to estimate the crude (unadjusted) relationship between `Mother_smoke` and `Birth_wt`?
...
**Q14** How could we plot the relationship between these two variables (`Mother_smoke` and `Birth_wt`)?
...
**Q14** Construct a linear model for the association between `Gestation` (independent variable) and birth weight  (`Birth_wt`; dependent variable), adjusted for ethnicity (`Ethnic`). Modify the code given below.
...
question("**Q16** Which of the two predictions has the widest interval?",
...
**Q17** What is the mean difference in birth weight of smokers' babies, compared to non-smokers', after accounting for confounders?
...
question("**Q19.** From the table above, describe the relationship between `Mother_age` and `Birth_wt`?",
...
**Q17** On average, after adjustment for confounders, how much weight does a baby put on for each additional week of gestation?
...
question("**Q18** The correct ending to the statement, 'On average, Pacific babies are heavier than…' is:?",
...
**Q19** "Extra for experts" question:
By how much heavier, on average, are Pacific babies compared to Māori babies (answer to the nearest whole number, in g)?
...
if(isTRUE(all.equal(.solution, .result, tolerance = 0) )){
    pass("Yes. Looks like you’re an expert! Compared to European, on average, Māori babies are 50.91 g lighter and 75.11 g heavier. So compared to Māori, Pacific babies will be 50.91 + 75.11 = 126.02 g heavier (on average)")
  }
  fail("Try again")
})
```

**Q20** In the code below, to produce model metrics change `metrics` from FALSE to TRUE. You will also need to specify the `dependent` and `explanatory` variables.
...
**Q21** What proportion of the overall variation in the data is explained by the model?
...
question("**Q22** Which terms in the model align with the data?",
